...
---------------------------------------------------------------------------
VENDORED. This file is an unmodified copy of Paul Tol's
``tol_colors.py`` (BSD-3), previously reached through ``from atelier import
tol_colors``. It was vendored here to drop the external ``atelier`` dependency
-- the only thing this repository ever used from that package was
``tol_cmap('rainbow_PuRd')``, and atelier is not pip-installable / not needed
for anything else. Import it as::

    from baqaro.utils import tol_colors

Upstream: https://personal.sron.nl/~pault/  (do not edit locally; re-vendor
from upstream if an update is wanted).
---------------------------------------------------------------------------
"""
import numpy as np
